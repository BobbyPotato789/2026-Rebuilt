...
public class DriveToGamePiece extends Command{
    ...
    private static final LoggedTunableNumber steerKp = new LoggedTunableNumber("Tuning/DriveToGP/SteerkP", 0.05);
    private static final LoggedTunableNumber steerKi = new LoggedTunableNumber("Tuning/DriveToGP/SteerkI", 0.0);
    private static final LoggedTunableNumber steerKd = new LoggedTunableNumber("Tuning/DriveToGP/SteerkD", 0.0);

    // Forward speed (also tunable) in m/s
    // private static final LoggedTunableNumber forwardSpeed = new LoggedTunableNumber("Tuning/DriveToGP/ForwardSpeed", 1.5);

    // Acceleration Limit (higher = snappier, lower = smoother)
    private static final LoggedTunableNumber maxAccel = new LoggedTunableNumber("Turning/DriveToGP/MaxAccel", 3.0);
    ...
    public void execute() {
        ...
        drive.runVelocity(
            new ChassisSpeeds(smoothedSpeed, 0.0, rotationOutput)
        );
    }
